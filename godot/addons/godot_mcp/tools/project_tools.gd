...
func get_errors(args: Dictionary) -> Dictionary:
	...
	for i: int in range(all_lines.size()):
		...
		# Godot continuation lines: "at: func_name (res://path/file.gd:123)"
		# Each error can have multiple frames — collect them all into a stack array.
		if not is_error and line.begins_with("at: ") and "res://" in line:
			if all_errors.size() > 0:
				var prev: Dictionary = all_errors[all_errors.size() - 1]
				var frame: Dictionary = _extract_stack_frame(line)
				if not frame.is_empty():
					prev[&"stack"].append(frame)
					# Also keep top-level file/line pointing at the innermost frame.
					if prev[&"stack"].size() == 1:
						prev[&"file"] = frame.get(&"file", "")
						prev[&"line"] = frame.get(&"line", 0)
			continue
		...
		var error_info: Dictionary = {&"message": line, &"severity": severity, &"stack": []}
...
func _extract_stack_frame(text: String) -> Dictionary:
	# text (already strip_edges'd): "at: func_name (res://file.gd:line)"
	var res_idx: int = text.find("res://")
	if res_idx == -1:
		return {}
	var func_name: String = ""
	var func_end: int = text.find(" (", 4)  # 4 = len("at: ")
	if func_end != -1 and func_end < res_idx:
		func_name = text.substr(4, func_end - 4).strip_edges()
	var loc: Dictionary = _extract_file_line(text)
	if loc.is_empty():
		return {}
	var frame: Dictionary = {&"func": func_name}
	frame.merge(loc)
	return frame
